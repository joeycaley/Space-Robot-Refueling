function [dirOfApr, deltaV, tElapsed] = interceptTraj(OE_g, OE_c, k)
% constants
...
if beta > 0
    ...
    if beta >= pi
        ...
        [deltaV, tElapsed] = outerOrbit(k,beta,Pog,nog,R,mu_Earth);
    else
        % need to speed up
        % inner orbit, approach from bottom
        dirOfApr = 0;

        [deltaV, tElapsed] = innerOrbit(k,beta,Pog,nog,R,mu_Earth);
    end
...
function [deltaV, tElapsed] = outerOrbit(k,beta,Pog,nog,R,mu_Earth)
...
function [deltaV, tElapsed] = innerOrbit(k,beta,Pog,nog,R,mu_Earth)
